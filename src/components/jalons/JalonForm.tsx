// ============================================================================
// FORMULAIRE JALON v3.0 - Design simplifié et convivial
// ============================================================================
...
import { useUsers, useJalons, useActions, createJalon, updateJalon } from '@/hooks';
...
export function JalonForm({ jalon, open, onClose, onSuccess }: JalonFormProps) {
    ...
    if (open) {
      if (jalon) {
        reset({
          axe: jalon.axe,
          titre: jalon.titre,
          cible: jalon.description || '',
          date_prevue: jalon.date_prevue,
          responsableId: jalon.responsableId || undefined,
          preuve_url: jalon.preuve_url || '',
          commentaire: jalon.commentaires?.[0]?.texte || '',
        });
        setShowOptional(!!(jalon.preuve_url || jalon.commentaires?.length));
      } else {
        reset({
          axe: 'axe3_technique',
          titre: '',
  ...
  const displayId = isEditing ? jalon?.id_jalon : generateJalonId(watchAxe);

  // Calculs
  const joursRestants = watchDate ? Math.ceil((new Date(watchDate).getTime() - Date.now()) / (1000 * 60 * 60 * 24)) : null;
  const actionsLiees = isEditing && jalon?.id ? actions.filter(a => a.jalonId === jalon.id) : [];
  const pourcentage = actionsLiees.length > 0 ? calculerPourcentageJalon(actionsLiees) : 0;
  const meteo: MeteoJalon = watchDate ? calculerMeteoJalon(pourcentage, new Date(new Date(watchDate).getTime() - 30 * 24 * 60 * 60 * 1000).toISOString(), watchDate) : 'SOLEIL';
  ...
  const onSubmit = async (data: JalonFormData) => {
    ...
    try {
      ...
      toast.success(isEditing ? 'Jalon mis a jour' : 'Jalon cree', `"${data.titre}" a ete enregistre`);
      onSuccess();
      onClose();
    } catch (error) {
        ...
        <DialogHeader>
          <DialogTitle className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <span>{isEditing ? 'Modifier le jalon' : 'Nouveau jalon'}</span>
              {displayId && (
            ...
            <div className="flex items-center gap-4 p-3 bg-neutral-50 rounded-lg text-sm">
              ...
              {isEditing && (
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-semibold text-blue-600">{pourcentage}%</span>
                </div>
              )}
              <div className="flex items-center gap-1">
          ...
          <DialogFooter className="gap-2 pt-4 border-t">
            ...
            >
              {isSubmitting ? (
                ...
                <><Save className="w-4 h-4 mr-1" />{isEditing ? 'Enregistrer' : 'Créer'}</>
              )}
            </Button>
          </DialogFooter>
        </form>

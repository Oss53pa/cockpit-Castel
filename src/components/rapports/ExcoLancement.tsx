...
import { SendReportModal } from './ExcoMensuelV5/SendReportModal';
import {
  Sun,
  CloudSun,
  Cloud,
...
// ============================================================================
// COULEURS - IDENTIQUES à l'EXCO Mensuel V5 (constants.ts)
// ============================================================================
const C = {
  // Navy (fond sombre)
  navy: '#263238',
...
const DESIGN_COLORS = {
  primary: '#263238',      // Navy - fond des pages de garde
  accent: '#B8953F',       // Or
  excellent: { color: '#10B981', bg: '#D1FAE5', border: '#10B981' },
  bon: { color: '#3B82F6', bg: '#DBEAFE', border: '#3B82F6' },
  attention: { color: '#F59E0B', bg: '#FEF3C7', border: '#F59E0B' },
  critique: { color: '#EF4444', bg: '#FEE2E2', border: '#EF4444' },
  neutral: { color: '#6B7280', bg: '#E5E7EB', border: '#D1D5DB' },
  textPrimary: '#111827',
...
// Configuration météo - Design Premium avec DESIGN_COLORS
const METEO_CONFIG: Record<MeteoType, { icon: React.ElementType; color: string; bgColor: string; borderColor: string }> = {
  soleil: { icon: Sun, color: DESIGN_COLORS.excellent.color, bgColor: DESIGN_COLORS.excellent.bg, borderColor: DESIGN_COLORS.excellent.border },
  soleil_nuage: { icon: CloudSun, color: DESIGN_COLORS.bon.color, bgColor: DESIGN_COLORS.bon.bg, borderColor: DESIGN_COLORS.bon.border },
  nuage: { icon: Cloud, color: DESIGN_COLORS.attention.color, bgColor: DESIGN_COLORS.attention.bg, borderColor: DESIGN_COLORS.attention.border },
  pluie: { icon: CloudRain, color: DESIGN_COLORS.critique.color, bgColor: DESIGN_COLORS.critique.bg, borderColor: DESIGN_COLORS.critique.border },
};
...
function MeteoBadge({ meteo, statut }: { meteo: MeteoType; statut: string }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-sm font-medium" style={{ color: config.color }}>{statut}</span>
    </div>
  );
}
...
function AxeMeteoBadge({ actions, jalons }: { actions: any[]; jalons: any[] }) {
  ...
  return (
    <div className="p-4 rounded-xl border bg-white" style={{ borderColor: config.borderColor }}>
      ...
      <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
        <div
          ...
          style={{
            width: `${meteoData.progress}%`,
            backgroundColor: meteoData.meteo === 'soleil' ? DESIGN_COLORS.excellent.color :
              meteoData.meteo === 'soleil_nuage' ? DESIGN_COLORS.bon.color :
              meteoData.meteo === 'nuage' ? DESIGN_COLORS.attention.color : DESIGN_COLORS.critique.color
          }}
        />
      </div>
    </div>
...
function PointsAttentionConsolides({ actions }: { actions: any[] }) {
  ...
  return (
    <Card padding="md" className="border" style={{ borderColor: DESIGN_COLORS.attention.border, backgroundColor: `${DESIGN_COLORS.attention.bg}4D` }}>
      <h4 className="font-semibold mb-3 flex items-center gap-2" style={{ color: DESIGN_COLORS.attention.color }}>
        ...
        {pointsAttention.slice(0, 5).map((pa) => (
          <div key={pa.id} className="p-2 bg-white rounded-lg border border-amber-200">
            ...
            <div className="flex items-center justify-between mt-1">
              ...
              {pa.responsableNom && (
                <span className="text-xs" style={{ color: DESIGN_COLORS.attention.color }}>{pa.responsableNom}</span>
              )}
            </div>
          </div>
        ))}
        {pointsAttention.length > 5 && (
          <p className="text-xs text-center" style={{ color: DESIGN_COLORS.attention.color }}>+{pointsAttention.length - 5} autres points</p>
        )}
      </div>
    </Card>
  );
...
function DecisionsAttenduesConsolidees({ actions }: { actions: any[] }) {
  ...
  return (
    <Card padding="md" className="border" style={{ borderColor: DESIGN_COLORS.bon.border, backgroundColor: `${DESIGN_COLORS.bon.bg}4D` }}>
      <h4 className="font-semibold mb-3 flex items-center gap-2" style={{ color: DESIGN_COLORS.primary }}>
      ...
      <div className="space-y-2">
        ...
        {decisionsAttendues.length > 5 && (
          <p className="text-xs text-center" style={{ color: DESIGN_COLORS.primary }}>+{decisionsAttendues.length - 5} autres decisions</p>
        )}
      </div>
    </Card>
  );
...
// ============================================================================
// SLIDE 0 - PAGE DE GARDE (Couverture)
// ============================================================================
function SlidePageGarde() {
  const kpis = useDashboardKPIs();
  const currentSite = useCurrentSite();
  const allSites = useSites();
    ...
    >
      ...
      <div>
        ...
        <div style={{ fontSize: 18, color: C.goldLight, fontWeight: 500 }}>
          {moisCourant.charAt(0).toUpperCase() + moisCourant.slice(1)}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginTop: 40 }}>
        <div style={{ backgroundColor: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: '20px 24px' }}>
          <div style={{ fontSize: 11, color: C.gray400, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>Ouverture</div>
          <div style={{ fontSize: 32, fontWeight: 700, color: C.gold, lineHeight: 1 }}>J-{joursRestants}</div>
          <div style={{ marginTop: 8, fontSize: 13, color: C.gray400 }}>{new Date(openingDate).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</div>
        </div>
        <div style={{ backgroundColor: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: '20px 24px' }}>
          <div style={{ fontSize: 11, color: C.gray400, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>Avancement Global</div>
          <div style={{ fontSize: 32, fontWeight: 700, color: C.gold, lineHeight: 1 }}>{Math.round(avancementGlobal)}%</div>
          <div style={{ marginTop: 8, fontSize: 13, color: C.gray400 }}>Cible 100%</div>
        </div>
        <div style={{ backgroundColor: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: '20px 24px' }}>
        ...
        <div style={{ backgroundColor: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: '20px 24px' }}>
          ...
          <div style={{ fontSize: 32, fontWeight: 700, color: C.gold, lineHeight: 1 }}>{actionsTerminees}/{totalActions}</div>
          <div style={{ marginTop: 8, fontSize: 13, color: C.gray400 }}>Terminées</div>
        </div>
      </div>

      {/* Bottom */}
      <div style={{ marginTop: 40, display: 'flex', justifyContent: 'flex-end', alignItems: 'flex-end' }}>
        ...
        <div className="space-y-2">
          {leftColumn.map((item) => (
            <div key={item.num} className="flex items-center gap-3 p-3 bg-white rounded-xl border border-slate-200 hover:border-indigo-300 hover:bg-slate-50 transition-all">
              <div className="w-7 h-7 rounded-lg text-white flex items-center justify-center text-sm font-semibold flex-shrink-0" style={{ backgroundColor: DESIGN_COLORS.primary }}>
                {item.num}
              </div>
    ...
    <div className="space-y-6">
      ...
      <Card padding="md" className="border-slate-200">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="text-center p-3 bg-slate-50 rounded-xl border border-slate-200">
            <p className="text-xl font-bold" style={{ color: DESIGN_COLORS.excellent.color }}>{actionsTerminees}</p>
            <p className="text-xs text-slate-500">Terminées</p>
          </div>
          <div className="text-center p-3 rounded-xl border" style={{ backgroundColor: DESIGN_COLORS.bon.bg, borderColor: DESIGN_COLORS.bon.border }}>
            <p className="text-xl font-bold" style={{ color: DESIGN_COLORS.primary }}>{avancement}%</p>
        ...
        <div className="flex items-center gap-4">
          <div className="flex-1">
            <div className="h-4 bg-gray-200 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${Math.min(tauxOccupationReel, 100)}%`,
                  backgroundColor: tauxOccupationReel >= tauxCible ? DESIGN_COLORS.excellent.color : DESIGN_COLORS.attention.color
                }}
              />
            </div>
          </div>
          <div className="text-right">
            <span
              ...
              style={{ color: tauxOccupationReel >= tauxCible ? DESIGN_COLORS.excellent.color : DESIGN_COLORS.attention.color }}
            >
  ...
  return (
    <div className="space-y-6">
      ...
      {risquesTechniques.length > 0 && (
        <div className="p-4 rounded-xl border" style={{ backgroundColor: `${DESIGN_COLORS.attention.bg}80`, borderColor: DESIGN_COLORS.attention.border }}>
          <div className="flex items-start gap-3">
            <div className="p-2 rounded-lg" style={{ backgroundColor: DESIGN_COLORS.attention.bg }}>
              <AlertTriangle className="h-4 w-4" style={{ color: DESIGN_COLORS.attention.color }} />
            </div>
            <div>
    ...
    <div className="space-y-6">
      ...
      <Card padding="md" className="border-slate-200">
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          ...
          <div className="text-center p-4 rounded-xl border" style={{ backgroundColor: DESIGN_COLORS.bon.bg, borderColor: DESIGN_COLORS.bon.border }}>
            <p className="text-lg font-bold" style={{ color: DESIGN_COLORS.primary }}>{formatMontantFCFA(budgetTotalRealise)}</p>
        ...
        <div className="w-full bg-slate-100 rounded-full h-2.5">
          <div
            ...
            style={{ backgroundColor: DESIGN_COLORS.primary, width: `${Math.min(tauxConsommation, 100)}%` }}
          />
        </div>
      </Card>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Card padding="md" className="text-center border" style={{ borderColor: DESIGN_COLORS.critique.border, backgroundColor: `${DESIGN_COLORS.critique.bg}80` }}>
          <p className="text-3xl font-bold" style={{ color: DESIGN_COLORS.critique.color }}>{totalCritiques}</p>
            ...
            {top5Risques.filter(r => (r.score || 0) >= 12).map((risque, idx) => (
              <div key={risque.id || idx} className="p-3 bg-slate-50 rounded-xl border-l-4 border-rose-500">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <Badge className="border" style={{ backgroundColor: DESIGN_COLORS.critique.bg, color: DESIGN_COLORS.critique.color, borderColor: DESIGN_COLORS.critique.border }}>Score: {risque.score}</Badge>
                </div>
          ...
          <div className="space-y-2">
            {pointsAttention.map((point) => (
              <div
                ...
                style={{
                  backgroundColor: pointsState[point.numero] ? `${DESIGN_COLORS.excellent.bg}80` : `${DESIGN_COLORS.attention.bg}80`,
                  borderColor: pointsState[point.numero] ? DESIGN_COLORS.excellent.border : DESIGN_COLORS.attention.border
                }}
              ...
              >
                <div className="flex items-center gap-3">
                  <span
                    ...
                    style={{ backgroundColor: pointsState[point.numero] ? DESIGN_COLORS.excellent.color : DESIGN_COLORS.attention.color }}
                  >
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      {point.responsablePoint && (
                        <span className="text-xs" style={{ color: DESIGN_COLORS.attention.color }}>Resp. point: {point.responsablePoint}</span>
                      )}
                ...
                <div className="flex items-center gap-2">
                  {pointsState[point.numero] ? (
                    <Badge className="border" style={{ backgroundColor: DESIGN_COLORS.excellent.bg, color: DESIGN_COLORS.excellent.color, borderColor: DESIGN_COLORS.excellent.border }}>Traite</Badge>
                  ) : (
                    <Badge variant="outline" style={{ borderColor: DESIGN_COLORS.attention.border, color: DESIGN_COLORS.attention.color, backgroundColor: DESIGN_COLORS.attention.bg }}>A traiter</Badge>
                  )}
                </div>
              </div>
            ))}
        ...
        <Card padding="md" className="border-slate-200">
          ...
          <div className="grid grid-cols-3 gap-2">
            {Object.entries(
              ...
              <div key={axe} className="p-2 rounded-lg border text-center" style={{ backgroundColor: DESIGN_COLORS.attention.bg, borderColor: DESIGN_COLORS.attention.border }}>
                <p className="text-xs truncate" style={{ color: DESIGN_COLORS.attention.color }} title={axe}>{axe}</p>
                <p className="text-lg font-bold" style={{ color: DESIGN_COLORS.attention.color }}>{count}</p>
              </div>
            ))}
          </div>
        </Card>
          ...
          <div className="space-y-2">
            {decisionsAttendues.map((decision) => (
              <div
                ...
                className="flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all"
                style={{
                  backgroundColor: decisionsState[decision.numero] ? `${DESIGN_COLORS.excellent.bg}80` : DESIGN_COLORS.bgCard,
                  borderColor: decisionsState[decision.numero] ? DESIGN_COLORS.excellent.border : DESIGN_COLORS.neutral.border
                }}
              ...
              >
                <div className="flex items-center gap-3">
                  <span
                    ...
                    style={{
                      backgroundColor: decisionsState[decision.numero] ? DESIGN_COLORS.excellent.color : DESIGN_COLORS.neutral.bg,
                      color: decisionsState[decision.numero] ? 'white' : DESIGN_COLORS.neutral.color
                    }}
                  >
                ...
                <div className="flex items-center gap-2">
                  {decisionsState[decision.numero] ? (
                    <Badge className="border" style={{ backgroundColor: DESIGN_COLORS.excellent.bg, color: DESIGN_COLORS.excellent.color, borderColor: DESIGN_COLORS.excellent.border }}>Valide</Badge>
                  ) : (
...
function SlideSynchronisation() {
  ...
  const getAlertConfig = () => {
    switch (alertLevel) {
      case 'GREEN':
        return { color: DESIGN_COLORS.excellent.color, bg: DESIGN_COLORS.excellent.bg, border: DESIGN_COLORS.excellent.border, label: 'Synchronise' };
      case 'ORANGE':
        return { color: DESIGN_COLORS.attention.color, bg: DESIGN_COLORS.attention.bg, border: DESIGN_COLORS.attention.border, label: 'Ecart modere' };
      case 'RED':
        return { color: DESIGN_COLORS.critique.color, bg: DESIGN_COLORS.critique.bg, border: DESIGN_COLORS.critique.border, label: 'Desynchronise' };
      default:
        return { color: DESIGN_COLORS.neutral.color, bg: DESIGN_COLORS.neutral.bg, border: DESIGN_COLORS.neutral.border, label: 'Non defini' };
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-4 rounded-xl border" style={{ backgroundColor: alertConfig.bg, borderColor: alertConfig.border }}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            {alertLevel === 'GREEN' && <CheckCircle className="h-6 w-6" style={{ color: alertConfig.color }} />}
            {alertLevel === 'ORANGE' && <AlertTriangle className="h-6 w-6" style={{ color: alertConfig.color }} />}
            {alertLevel === 'RED' && <AlertTriangle className="h-6 w-6" style={{ color: alertConfig.color }} />}
            <div>
              <p className="font-semibold" style={{ color: alertConfig.color }}>{alertConfig.label}</p>
              <p className="text-sm text-slate-500">Ecart de synchronisation: {gap > 0 ? '+' : ''}{gap.toFixed(1)}%</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {gap > 0 ? (
              <TrendingUp className="h-5 w-5" style={{ color: DESIGN_COLORS.primary }} />
            ) : gap < 0 ? (
              <TrendingDown className="h-5 w-5" style={{ color: DESIGN_COLORS.attention.color }} />
            ) : (
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card padding="md" className="border-slate-200 text-center">
          <div className="p-2 rounded-xl w-fit mx-auto mb-2" style={{ backgroundColor: DESIGN_COLORS.bon.bg }}>
            <Building2 className="h-5 w-5" style={{ color: DESIGN_COLORS.primary }} />
          </div>
        ...
        <Card padding="md" className="border-slate-200 text-center">
          <div className="p-2 rounded-xl w-fit mx-auto mb-2" style={{ backgroundColor: DESIGN_COLORS.excellent.bg }}>
            <Users className="h-5 w-5" style={{ color: DESIGN_COLORS.excellent.color }} />
          </div>
        ...
        <Card padding="md" className="text-center border" style={{ borderColor: alertConfig.border }}>
          <div className="p-2 rounded-xl w-fit mx-auto mb-2" style={{ backgroundColor: alertConfig.bg }}>
            <Link2 className="h-5 w-5" style={{ color: alertConfig.color }} />
          </div>
          <p className="text-2xl font-bold" style={{ color: alertConfig.color }}>{gap > 0 ? '+' : ''}{gap.toFixed(1)}%</p>
      ...
      <Card padding="md" className="border-slate-200">
        ...
        <div className="flex items-center justify-around">
          <div className="relative">
            <CircularGauge value={constructionProgress} label="Construction" color={DESIGN_COLORS.primary} />
          </div>
          <div className="flex flex-col items-center px-4">
            <div className="p-3 rounded-full" style={{ backgroundColor: alertConfig.bg }}>
              <Link2 className="h-6 w-6" style={{ color: alertConfig.color }} />
            </div>
            <span className="text-sm font-medium mt-2" style={{ color: alertConfig.color }}>
              {gap > 0 ? 'Avance' : gap < 0 ? 'Retard' : 'Sync'}
            </span>
          </div>
          <div className="relative">
            <CircularGauge value={mobilisationProgress} label="Mobilisation" color={DESIGN_COLORS.excellent.color} />
          </div>
        </div>
      </Card>
      ...
      <Card padding="md" className="border-slate-200">
        ...
        <div className="grid grid-cols-4 gap-3">
          ...
          <div className="text-center p-3 rounded-xl border" style={{ backgroundColor: DESIGN_COLORS.excellent.bg, borderColor: DESIGN_COLORS.excellent.border }}>
            <p className="text-xl font-bold" style={{ color: DESIGN_COLORS.excellent.color }}>{syncStats?.actionsTerminees || syncActionsStats.terminees}</p>
            <p className="text-xs text-slate-500">Terminees</p>
          </div>
          <div className="text-center p-3 rounded-xl border" style={{ backgroundColor: DESIGN_COLORS.bon.bg, borderColor: DESIGN_COLORS.bon.border }}>
            <p className="text-xl font-bold" style={{ color: DESIGN_COLORS.primary }}>{syncActionsStats.enCours}</p>
            <p className="text-xs text-slate-500">En cours</p>
          </div>
          <div className="text-center p-3 rounded-xl border" style={{ backgroundColor: DESIGN_COLORS.attention.bg, borderColor: DESIGN_COLORS.attention.border }}>
            <p className="text-xl font-bold" style={{ color: DESIGN_COLORS.attention.color }}>{syncActionsStats.enRetard}</p>
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <span className="text-sm font-medium text-slate-600 flex items-center gap-2">
                <Building2 className="h-4 w-4" style={{ color: DESIGN_COLORS.primary }} /> Construction
              </span>
              <span className="text-sm font-bold" style={{ color: DESIGN_COLORS.primary }}>{constructionProgress.toFixed(0)}%</span>
            </div>
            <div className="h-3 bg-slate-100 rounded-full overflow-hidden">
              <div
                className="h-full rounded-full transition-all"
                style={{ width: `${constructionProgress}%`, backgroundColor: DESIGN_COLORS.primary }}
              />
            </div>
          </div>
          <div>
            <div className="flex items-center justify-between mb-1">
              <span className="text-sm font-medium text-slate-600 flex items-center gap-2">
                <Users className="h-4 w-4" style={{ color: DESIGN_COLORS.excellent.color }} /> Mobilisation
              </span>
              <span className="text-sm font-bold" style={{ color: DESIGN_COLORS.excellent.color }}>{mobilisationProgress.toFixed(0)}%</span>
            </div>
            <div className="h-3 bg-slate-100 rounded-full overflow-hidden">
              <div
                className="h-full rounded-full transition-all"
                style={{ width: `${mobilisationProgress}%`, backgroundColor: DESIGN_COLORS.excellent.color }}
              />
            </div>
          </div>
        </div>
...
function SlideConclusion() {
  return (
    ...
    >
      ...
      {/* Thank you */}
      <h1 style={{ fontSize: 42, fontWeight: 600, marginBottom: 24, color: C.white }}>
        Merci de votre attention
      </h1>

      {/* Decorative line */}
      <div style={{ width: 80, height: 4, marginBottom: 24, borderRadius: 2, backgroundColor: C.gold }} />

      {/* Project Name */}
      <h2 style={{ fontSize: 28, fontWeight: 500, marginBottom: 8, color: C.gray400 }}>
        {PROJET_CONFIG.nom}
      </h2>
      <p style={{ fontSize: 18, marginBottom: 40, color: C.goldLight }}>
        EXCO Report
      </p>
...
export function ExcoLancement() {
  ...
  // États pour le preview comme EXCO normal
  ...
  const slides = [
    { numero: 1, titre: 'Page de Garde', component: SlidePageGarde },
    { numero: 2, titre: 'Agenda', component: SlideAgenda },
    { numero: 3, titre: 'Rappel Projet & Météo Globale', component: SlideRappelProjet },
    { numero: 4, titre: 'AXE RH & Organisation', component: SlideAxeRH },
    { numero: 5, titre: 'AXE Commercial & Leasing', component: SlideAxeCommercial },
    { numero: 6, titre: 'AXE Technique & Handover', component: SlideAxeTechnique },
    { numero: 7, titre: 'AXE Budget & Finances', component: SlideAxeBudget },
    { numero: 8, titre: 'AXE Marketing & Communication', component: SlideAxeMarketing },
    { numero: 9, titre: 'AXE Exploitation & Juridique', component: SlideAxeExploitation },
    { numero: 10, titre: 'Synchronisation Chantier / Mobilisation', component: SlideSynchronisation },
    { numero: 11, titre: 'Risques Majeurs', component: SlideRisquesMajeurs },
    { numero: 12, titre: "Points d'Attention", component: SlidePointsAttention },
    { numero: 13, titre: 'Décisions Attendues', component: SlideDecisions },
    { numero: 14, titre: 'Conclusion', component: SlideConclusion },
  ];
...
<head>
  ...
  <title>EXCO Report - Cosmos Angré - ${dateFormatted}</title>
  ...
  <style>
    ...
    .meteo-soleil { color: #10B981; background: #D1FAE5; }
    .meteo-nuage { color: #F59E0B; background: #FEF3C7; }
    .meteo-pluie { color: #EF4444; background: #FEE2E2; }
    .meteo-soleil-nuage { color: #3B82F6; background: #DBEAFE; }
  </style>
</head>
<body class="bg-gray-100">
  ...
  <div class="bg-[#1C3163] text-white p-4 mt-8 no-print">
    <div class="max-w-6xl mx-auto text-center text-sm">
      <p>EXCO Report - Cosmos Angré - Cosmos Angré</p>
  ...
  const downloadHtml = () => {
    ...
    a.href = url;
    a.download = `exco-lancement-${presentationDate}.html`;
  ...
  // Générer PowerPoint (même qualité que EXCO Mensuel)
  const generatePowerPoint = async () => {
    ...
    try {
      ...
      pptx.title = `EXCO Report - Cosmos Angré - ${dateFormatted}`;
      pptx.subject = 'Validation Stratégique Direction Générale';
      pptx.company = 'Cosmos Angré / Cosmos Angré';
      ...
      endSlide.addText(`${PROJET_CONFIG.nom} - ${PROJET_CONFIG.nom}`, {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-primary-900">EXCO Report</h1>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button size="sm" onClick={() => setShowSendModal(true)} className="bg-primary-600 text-white hover:bg-primary-700">
      ...
      <Card padding="lg">
        ...
        <div className="flex items-center justify-between mt-6 pt-4 border-t">
          ...
          <span className="text-sm text-primary-500">
            EXCO Report - Cosmos Angré - Cosmos Angré
          </span>
          <Button
            variant="outline"
          ...
          <div className={cn("bg-gray-100 rounded-lg p-4 flex items-center justify-center", isFullscreen ? "flex-1" : "flex-1 min-h-0")}>
            ...
            >
              ...
              <div className="flex-shrink-0 bg-gray-50 px-6 py-2 border-t flex items-center justify-between text-xs text-gray-500">
                <span>Cosmos Angré - Confidentiel</span>

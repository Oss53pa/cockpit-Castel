...
import type { DetailAxeData, MeteoNiveau } from '@/types/exco';
...
export function DetailAxeSlide({ data, designSettings, periode }: DetailAxeSlideProps) {
  ...
  const formatMontant = (montant: number): string => {
    if (montant >= 1_000_000) {
      return `${(montant / 1_000_000).toFixed(1)}M ${PROJET_CONFIG.devise}`;
    }
    if (montant >= 1_000) {
      return `${(montant / 1_000).toFixed(0)}K ${PROJET_CONFIG.devise}`;
    }
    return `${montant} ${PROJET_CONFIG.devise}`;
  };

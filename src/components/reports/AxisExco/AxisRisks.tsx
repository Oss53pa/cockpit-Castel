...
import { SEUILS_RISQUES, SEUILS_UI } from '@/data/constants';
...
const scoreConfig = {
  critique: { min: SEUILS_RISQUES.critique, label: 'Critique', color: 'text-error-600', bgColor: 'bg-error-100' },
  majeur: { min: SEUILS_RISQUES.majeur, label: 'Majeur', color: 'text-orange-600', bgColor: 'bg-orange-100' },
  modere: { min: SEUILS_RISQUES.modere, label: 'Modéré', color: 'text-warning-600', bgColor: 'bg-warning-100' },
  faible: { min: 0, label: 'Faible', color: 'text-success-600', bgColor: 'bg-success-100' },
};

const getScoreLevel = (score: number) => {
  if (score >= SEUILS_RISQUES.critique) return scoreConfig.critique;
  if (score >= SEUILS_RISQUES.majeur) return scoreConfig.majeur;
  if (score >= SEUILS_RISQUES.modere) return scoreConfig.modere;
...
export function AxisRisks({ risques, axeColor }: AxisRisksProps) {
  ...
  const stats = useMemo(() => {
    ...
    const critiques = actifs.filter(r => {
      ...
      return score >= SEUILS_RISQUES.critique;
    });
    ...
    <Card padding="md">
      ...
      <div className="mb-4 p-4 bg-gray-50 rounded-lg">
        ...
        <div className="h-4 bg-gray-200 rounded-full overflow-hidden flex">
          {Object.entries({
            critique: risques.filter(r => getScore(r) >= SEUILS_RISQUES.critique && r.status !== 'ferme').length,
            majeur: risques.filter(r => getScore(r) >= SEUILS_RISQUES.majeur && getScore(r) < SEUILS_RISQUES.critique && r.status !== 'ferme').length,
            modere: risques.filter(r => getScore(r) >= SEUILS_RISQUES.modere && getScore(r) < SEUILS_RISQUES.majeur && r.status !== 'ferme').length,
            faible: risques.filter(r => getScore(r) < SEUILS_RISQUES.modere && r.status !== 'ferme').length,
          }).map(([level, count]) => {
      ...
      <div className="overflow-x-auto">
        <Table>
          ...
          <TableBody>
            {sortedRisques.slice(0, SEUILS_UI.topRisques).map((risque) => {

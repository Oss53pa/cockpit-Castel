/**
 * AxisHeader - En-tête du EXCO avec score santé et météo de l'axe
 */
...
export function AxisHeader({
  ...
  const metrics = useMemo(() => {
    ...
    const scoreAvancement = Math.min(100, (avancementRealise / Math.max(1, avancementPrevu)) * SEUILS_SANTE_AXE.poids.avancement);
    const scoreActions = Math.max(0, SEUILS_SANTE_AXE.poids.actions - (actionsEnRetard * SEUILS_SANTE_AXE.penalites.actionEnRetard) - (actionsBloquees * SEUILS_SANTE_AXE.penalites.actionBloquee));
    const scoreRisques = Math.max(0, SEUILS_SANTE_AXE.poids.risques - (risquesCritiques * SEUILS_SANTE_AXE.penalites.risqueCritique));
    ...
    if (scoreSante < SEUILS_SANTE_AXE.meteo.pluie.score || actionsEnRetard >= SEUILS_SANTE_AXE.meteo.pluie.actionsEnRetard || risquesCritiques >= SEUILS_SANTE_AXE.meteo.pluie.risquesCritiques) {
      meteo = 'pluie';
    } else if (scoreSante < SEUILS_SANTE_AXE.meteo.nuage.score || actionsEnRetard >= SEUILS_SANTE_AXE.meteo.nuage.actionsEnRetard || risquesCritiques >= SEUILS_SANTE_AXE.meteo.nuage.risquesCritiques) {
      meteo = 'nuage';
    } else if (scoreSante < SEUILS_SANTE_AXE.meteo.soleilNuage.score || actionsEnRetard >= SEUILS_SANTE_AXE.meteo.soleilNuage.actionsEnRetard || risquesCritiques >= SEUILS_SANTE_AXE.meteo.soleilNuage.risquesCritiques) {
      meteo = 'soleil_nuage';
    }
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-primary-500">EXCO détaillé</p>
          </div>
        </div>

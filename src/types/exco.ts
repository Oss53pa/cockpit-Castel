...
// Template types for different EXCO formats
export type ExcoTemplateType = 'launch' | 'monthly';

export interface ExcoTemplate {
  type: ExcoTemplateType;
  label: string;
  description: string;
...
export interface ExcoDesignSettings {
  primaryColor: string;
  accentColor: string;
  fontFamily: string;
...
export interface Exco {
  ...
  description?: string;
  projectName: string;
  templateType: ExcoTemplateType;
  weather: ProjectWeather;
  kpis: ExcoKPIs;
  designSettings: ExcoDesignSettings;
  slides: ExcoSlide[];
  decisionPoints: DGDecisionPoint[];
  axesData: ExcoAxeData[];
  topRisks: ExcoRisk[];
  upcomingJalons: ExcoJalon[];
  createdAt: string;
  updatedAt: string;
  createdBy: string;
  ...
  // Champs pour le lien Générateur → EXCO V5 → Journal
  v5DataSnapshot?: string;  // JSON sérialisé de ExcoV5Data (sans fonctions)
  source?: 'generator' | 'manual' | 'exco_tab';
  generatedReportId?: string;  // lien vers le rapport source

  // Champs utilisés par le Journal (schéma libre IndexedDB)
  meteoGlobale?: ProjectWeather;
  projet?: string;
  auteur?: string;
  periodeDebut?: string;
  periodeFin?: string;
  axes?: Record<string, { actif: boolean; meteo: string; commentaire: string }>;
  faitsMarquants?: string[];
  risques?: string[];
  budgetCommentaire?: string;
  planningCommentaire?: string;
...
// EXCO Template configurations
export const EXCO_TEMPLATES: Record<ExcoTemplateType, ExcoTemplate> = {
  launch: {
    type: 'launch',
    label: 'EXCO de Lancement & Cadrage',
...
// ============================================================================
// EXCO MENSUEL V2 - TYPES CONFORMES AU TEMPLATE COPIL
// ============================================================================
...
// Structure hiérarchique des sections du EXCO Mensuel
export interface ExcoMensuelSection {
  id: string;
  numero: string; // "1", "2", "3", etc.
  titre: string;
  type: 'synthese' | 'analyse_axe' | 'risques' | 'decisions' | 'plan_action' | 'annexes';
  slides: ExcoMensuelSlide[];
  expanded?: boolean;
}

export interface ExcoMensuelSlide {
  id: string;
  sectionId: string;
...
// Structure complète du EXCO Mensuel V2
export interface ExcoMensuelV2 {
  id?: number;
  titre: string;
  projectName: string;
  periode: string;
